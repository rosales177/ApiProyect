...
class ClientController extends BaseController {
    ...
    public function modifyClient($request,$response,$args){
        try{
            ...
            // $Img = $body['Img'];
            $conn = $this->container->get('db');
            $sql = "CALL sp_UpdateCliente(:IdClient,:NombreClient,:ApellidoClient,:EdadClient,:CorreoClient,:ContactoClient)";
            $stm = $conn->prepare($sql);
    ...
    public function modifyClientImg($request,$response,$args){
        try{
            $params =intval($args['id']);
            $body = json_decode($request->getBody(), true);
            $Cliente_img =$body['Img'];
            $conn = $this->container->get('db');
            $sql = "UPDATE CLIENTE SET `Img` = :Cliente_img  WHERE `id_Cliente` = :IdClient";
            $stm = $conn->prepare($sql);
            $stm->bindParam(':IdClient',$params);
            $stm->bindParam(':Cliente_img',$Cliente_img);
            $stm->execute();
            $result = $stm->fetchAll();
            return $this->jsonResponse($response,'success',$result,200);
        }catch(Exception $e){
